...
import com.enterlib.mvvm.IEditableView;
import com.enterlib.mvvm.IFormView;
...
public class FormFragment extends FragmentView implements INotifyPropertyChanged, IFormView ,IEditableView{
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if(form!=null) {
			...
			if(readonlyFields == null){
				...
				if(args!=null && args.containsKey(READ_ONLY_FIELDS))
					readonlyFields = args.getStringArray(READ_ONLY_FIELDS);
			}

			disableFields(readonlyFields);
		}
	}

	public void disableFields(String... readonlyFields) {
	...
	public void disableFields(ArrayList<String> readonlyFields) {
		if (readonlyFields != null && form != null) {
			for (int i = 0; i < readonlyFields.size(); i++) {
				Field field = form.getFieldByBinding(readonlyFields.get(i));
				if (field != null)
					field.setEnabled(false);
			}
		}
	...
	public boolean updateSources(Object viewModel) {
		if (form == null) {
			return false;
		}
        try{
